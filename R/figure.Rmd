...
top.dist2 <- ggplot(data=df2, aes(x = z, y=density)) +
  ...
  geom_line(data=filter(df2,z>=2),linetype="solid", color="#E7298A", linewidth = 0.8) +
  geom_ribbon(data=filter(df2,z>=2),aes(z, ymax=upper_ci, ymin=lower_ci), fill="#E7298A", alpha = 0.2) +
  geom_segment(aes(x = 2 , y = 0, xend = 2, yend = 0.185), linetype = "dashed", color = "grey") + 
  
  scale_x_continuous(limits = c(0, 10), breaks = seq(0, 10, by = 2), expand = expansion(mult = c(0, 0.025))) + 
...
png(filename = here("figure","fig 1.png"), width = 6, height = 5, units = "in", type = "windows", res = 400)
plot_grid(top.dist2,
          replicate.dist,
          align = "v",
          ncol = 1,
...
evidence.rep.p <- ggplot(df, aes(x= strength, y = replicate, ymin=lower_ci, ymax=upper_ci, color = evidence, fill = evidence)) + 
  ...
  #geom_text(aes(label = evidence2), size = 3.5, color = "#A6761D", vjust = -3) +
  geom_text(aes(label = evidence3), size = 3.5, color = "#A6761D", vjust = 3) + 
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.2),
...
replicate_multiplier.p <- df %>% ggplot() +
  ...
  annotate("text", x = 3, y = 0.50, label = "Weak evidence", size = 5, color = "#A6761D") +
  #annotate("text", x = 3, y = 0.75, label = "Moderate evidence", size = 4, color = "#A6761D") +
  annotate("text", x = 3, y = 0.84, label = "Strong evidence", size = 5, color =  "#A6761D")


png(filename = here("figure","fig 2.png"), width = 7, height = 11, units = "in", type = "windows", res = 400)
